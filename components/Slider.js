        ...
        {slide.map((images, imagesIndex) => {
          ...
          return (
            <div className={`silde ${position}`} key={id}>
              <img style={{background: 'rgba(255,255,255,0.1))'}} src={image} alt={id} className="food-img" />
            </div>
          );
        })}
        <button className="prev" onClick={() => setIndex(index - 1)}>
          <img style={{background: 'rgba(255,255,255,0.1))'}} src={left} alt="" />
        </button>
        <button className="next" onClick={() => setIndex(index + 1)}>
          <img style={{background: 'rgba(255,255,255,0.1))'}} src={right} alt="" />
        </button>
      </div>
    </section>
  );
